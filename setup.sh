...
if [ "$config_file_found" = "true" ]; then
    ...
    if ! grep -q "$config_script_dir" "$script_dir/config.ini"; then  
        sed -i "s|programFolder: .*|programFolder: $config_script_dir|" config.ini
...
database_files=( $(find "$script_dir" -name "*.db") )
...
if [ ${#database_files[@]} -eq 0 ]; then
    ...
    if [ "$download_db" = "Y" ] || [ "$download_db" = "y" ]; then
        if command -v curl &>/dev/null; then
            ...
            curl -o "$script_dir/database.db" "$github_raw_url"
...
if [ -e "$script_dir/config.ini" ] && [ -e "$script_dir/database.db" ]; then
