...
active_orders = tm.get_active_orders()
...
class Menu:
	def __init__(self, root): # initialise the gui
		...
		try:
			...
			command=lambda: self.view_order_for_day("active"))
	...
	def button_total_clicked(self):
		...
		try:
			...
			self.show_warning("Customer Name Invalid", 0)

		receipt = tm.do_receipt(customer_name) # this function sorts out the receipt for my records and gets the total price for the transaction
		...
		tm.update_active_orders() # update the list of orders that are currently active so they can be managed

	def remove_selected_line(self):
		
		tm.remove_last_item() # removes the last items behind the scenes
	...
	def new_order(self):
		...
		try: 
			curOrders = tm.active_order_count()
			...
			tm.new_order() # starts the new order 
			...
			number = tm.give_order_number() # gets the order number to put at the top of the receipt
	...
	def pizza_customisation_menu(self, pizza_size):
		...
		def custom_option_clicked(option_text, pizza_size):
			...
			text = tm.get_pizza_price(pizza_size, option_text) # gets the price from my pizza database
	...
	def view_order_for_day(self, date):
	
		if date == "active":
			orders = tm.get_active_orders()
		else:
			orders = tm.return_legacy_orders(date)

		try:
			if len(orders) == 0:
				self.show_warning("No Active Orders.", 0)    
			else:    
				active_orders_window = tk.Toplevel(self.root)
				active_orders_window.title("Orders")
				...
				for index, order in enumerate(orders, start=1):
					...
					clear_order_lambda = lambda order=order: self.clear_order(order, active_orders_window, date)
					view_order_lambda = lambda order=order: self.view_order(order, date)
	...
	def view_order(self, order, date):
		...
		new_window.title(f"{order.capitalize()}")

		orderLines = tm.get_order_to_view(order, date)
	...
	def clear_order(self, order, active_orders_window, date):

		tm.clear_selected_order(order)
		tm.update_active_orders()
		active_orders_window.destroy()
		self.view_order_for_day(date)
	...
	def sauce_customisation_menu(self, sauce_size):
		...
		def custom_option_clicked(option_text):
			...
			text = tm.custom_sauce(sauce_size, option_text)
	...
	def admin_menu(self):
		...
		correct_password_hash = tm.get_admin_password()
	...
	def total_takings(self):
		total_price = tm.get_todays_takings()
	...
	def daily_report(self):
		...
		def get_selected_date():
			...
			try:
				report, total_price = tm.create_report(date)
		...
		year, month, day = tm.get_current_date()        
	...
	def view_all_orders_from_day(self, date):
